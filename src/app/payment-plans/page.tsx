...
import PayPlanTable from "@/layout/payment-plans/PayPlanTable";
    ...
    const getorder = async () => {
      try {
        ...
        const { data } = await api.get(
          `/admin/orders-with-subscription-bought-products?limit=${limit}&page=${page}&start_date=${s_date}&end_date=${e_date}`
        );
        setloading(false);
  ...
  return (
    <CommonLayout>
      {!loading ? (
        ...
        >
          {orders && <PayPlanTable orders={orders} />}
        </PaymentOrdersLayout>
      ) : (
        <Loader />
      )}
